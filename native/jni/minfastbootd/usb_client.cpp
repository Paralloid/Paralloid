...
static bool InitFunctionFs(usb_handle* h) {
    ...
    if (h->control < 0) {  // might have already done this before
        std::cout << "opening control endpoint " << kUsbFfsFastbootEp0 << std::endl;
        h->control.reset(open(kUsbFfsFastbootEp0, O_RDWR));
        if (h->control < 0) {
            std::cout << "cannot open control endpoint " << kUsbFfsFastbootEp0 << std::endl;
            goto err;
        }

        auto ret = write(h->control.get(), &v2_descriptor, sizeof(v2_descriptor));
        if (ret < 0) {
            std::cout << "cannot write descriptors " << kUsbFfsFastbootEp0 << std::endl;
            goto err;
        }

        ret = write(h->control.get(), &strings, sizeof(strings));
        if (ret < 0) {
            std::cout << "cannot write strings " << kUsbFfsFastbootEp0 << std::endl;
            goto err;
        }
    }
    ...
    if (h->bulk_out < 0) {
        std::cout << "cannot open bulk-out endpoint " << kUsbFfsFastbootOut << std::endl;
        goto err;
    }

    h->bulk_in.reset(open(kUsbFfsFastbootIn, O_WRONLY));
    if (h->bulk_in < 0) {
        std::cout << "cannot open bulk-in endpoint " << kUsbFfsFastbootIn << std::endl;
        goto err;
    }
